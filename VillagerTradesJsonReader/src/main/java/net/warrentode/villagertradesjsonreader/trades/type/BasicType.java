package net.warrentode.villagertradesjsonreader.trades.type;
...
import net.warrentode.villagertradesjsonreader.trades.BasicTrade;
import net.warrentode.villagertradesjsonreader.trades.TradeSerializer;
...
import static net.warrentode.villagertradesjsonreader.VillagerTradesJSONReader.MODID;

// AUTHOR: MrCrayfish https://github.com/MrCrayfish/GoblinTraders/tree/1.19.X
public class BasicType implements ITradeType<BasicTrade> {
    ...
    public BasicType(ItemStack offerStack, ItemStack paymentStack, ItemStack secondaryPaymentStack, float priceMultiplier, int maxTrades, int experience, EnchantmentInstance[] enchantments, Collection<MobEffectInstance> mobEffects) {
    ...
    @Override
    public BasicTrade createVillagerTrade() {
        ItemStack offerStack = this.offerStack.copy();
        if (this.enchantments.length > 0) {
            if (offerStack.getItem() == Items.ENCHANTED_BOOK) {
                EnchantmentHelper.setEnchantments(Stream.of(this.enchantments).collect(Collectors.toMap(enchantmentInstance -> enchantmentInstance.enchantment, enchantmentInstance2 -> enchantmentInstance2.level)), offerStack);
            }
            else {
        ...
        return new BasicTrade(offerStack, this.paymentStack.copy(), this.secondaryPaymentStack.copy(), this.maxTrades, this.experience, this.priceMultiplier);
    }
    ...
    public static class Serializer extends TradeSerializer<BasicType> {
        Serializer() {
            super(new ResourceLocation(MODID, "basic"));
        }

        @Override
        public BasicType deserialize(JsonObject object) {
        ...
        @Override
        public JsonObject serialize(BasicType trade) {
    ...
    public static class Builder {
        ...
        public BasicType build() {
            return new BasicType(this.offerStack, this.paymentStack, this.secondaryPaymentStack, this.priceMultiplier, this.maxTrades, this.experience, this.enchantments.toArray(new EnchantmentInstance[0]), this.modEffects);
        }
