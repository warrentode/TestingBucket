...
public class InteractWithDoor extends Behavior<LivingEntity> {
    ...
    public static void closeDoorsThatIHaveOpenedOrPassedThrough(ServerLevel pLevel, LivingEntity pEntity, @Nullable Node p_23301_, @Nullable Node p_23302_) {
                ...
                if ((p_23301_ == null || !p_23301_.asBlockPos().equals(blockpos)) && (p_23302_ == null || !p_23302_.asBlockPos().equals(blockpos))) {
                    if (isDoorTooFarAway(pLevel, pEntity, globalpos)) {
                        ...
                        if (!blockstate.is(BlockTags.WOODEN_DOORS, (p_201952_) -> {
                            ...
                            if (!doorblock.isOpen(blockstate)) {
                                iterator.remove();
                            } else if (areOtherMobsComingThroughDoor(pLevel, pEntity, blockpos)) {
                                iterator.remove();
                            } else {
    ...
    private void rememberDoorToClose(ServerLevel pLevel, LivingEntity pEntity, BlockPos pPos) {
        Brain<?> brain = pEntity.getBrain();
